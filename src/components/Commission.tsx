...
import { tiers, budgets } from "../data";
...
export function Commission() {
  ...
  const [errors, setErrors] = useState<{ character?: string; budget?: string }>({});
  ...
  const onSubmit = (e: FormEvent) => {
    ...
    // Front-end demo — swap for a Formspree/Getform endpoint to receive briefs.
      ...
      <div className="mx-auto grid max-w-[1240px] items-start gap-12 px-5 py-24 sm:px-8 md:grid-cols-[1fr_1.05fr] md:gap-20 md:py-32">
        <header className="md:sticky md:top-28">
          ...
          <p className="mt-5 max-w-[44ch] text-[var(--muted)]">
            Ceritakan karakter dan budget-mu, nanti kami bantu carikan bahan yang pas.
            Pengukuran menyusul setelah kita sepakat.
          </p>
        </header>
          ...
          <AnimatePresence mode="wait">
            {status === "success" ? (
              ...
              >
                ...
                <p className="mt-3 max-w-[34ch] text-[var(--muted)]">
                  Terima kasih! Pesanan untuk <span className="text-[var(--silk)]">{character.trim()}</span>{" "}
                  sudah kami catat. Kami balas dalam 3 hari kerja, ya.
                </p>
                <button
                  type="button"
                  onClick={() => {
              ...
              >
                    ...
                    {tiers.map((t) => {
                      ...
                      return (
                        <button
                          ...
                          aria-checked={active}
                          onClick={() => setTier(t)}
                          className={`rounded-full border px-[1.05rem] py-[0.6rem] text-[0.8rem] transition-all duration-300 active:scale-[0.97] ${
                            active
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <textarea
                    id="notes"
                    rows={3}
                    placeholder="Referensi gambar, pilihan kain, detail karakter…"
